/*
 * Oregon Scientific THN132N (EC40) - VERSIÓ UNIVERSAL (ESP32 + AHT20)
...
bool aht20_init() {
  ...
  if (!(status & 0x08)) { // Calibrated bit not set
    Wire.beginTransmission(AHT20_ADDR);
    Wire.write(0xBE);
    Wire.write(0x08);
    Wire.write(0x00);
...
bool aht20_read(float &temp, float &hum) {
  ...
  // Read data
  Wire.requestFrom(AHT20_ADDR, 7); // Status + 6 bytes data
  ...
  if (status & 0x80) return false; // Busy

  // Calculate Humidity
  uint32_t rawHum = ((uint32_t)data[0] << 12) | ((uint32_t)data[1] << 4) | (data[2] >> 4);
  hum = (float)rawHum * 100.0 / 1048576.0;
...
void setup() {
  ...
  Serial.println();
  Serial.println(F("=============================================="));
  Serial.println(F("Oregon THN132N - VERSIÓ UNIVERSAL (AHT20)"));
  Serial.println(F("Amb taules universals descobertes"));
  Serial.println(F("=============================================="));
  
  if (!aht20_init()) {
    Serial.println(F("Error: AHT20 no detectat!"));
  } else {
    Serial.println(F("AHT20 inicialitzat correctament."));
  }
...
void loop() {
  ...
  if (now - lastSend >= periodMs) {
    ...
    float temp_c = 0, hum = 0;
    if (!aht20_read(temp_c, hum)) {
      Serial.println(F("Error: No s'ha pogut llegir el sensor AHT20"));
      // Intentar re-init
      aht20_init();
      return;
    }

    // Arrodonir a 1 decimal
    temp_c = roundf(temp_c * 10.0f) / 10.0f;
