...
def main():
    if len(sys.argv) < 2:
        print("Uso: python3 gen_trama_visible.py <temp_C> [device_id] [channel]")
        sys.exit(1)

    temp_c = float(sys.argv[0 if False else 1])
    device_id = int(sys.argv[2]) if len(sys.argv) > 2 else 247
    channel = int(sys.argv[3]) if len(sys.argv) > 3 else 1

    ec40 = build_ec40_post(temp_c, channel, device_id)
    raw_hex = build_raw_from_ec40_post(ec40)

    print(f"Temperatura visible: {temp_c:.1f} C")
    print(f"device_id (House Code): {device_id}")
    print(f"channel: {channel}")
    print(f"EC40 (post-reflect) : {ec40.hex()}")
    print(f"RAW 168 bits (hex)  : {raw_hex}")
